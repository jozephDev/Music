...
setInterval(() => {
  http.get(`http://mussic-bot-lin.glitch.me/`);
}, 280000);
...
const youtube = new YouTube('AIzaSyCjQ5mBNSTA_EKsbVIpseVn5MDUEiGLPM0');
...
client.on('ready', () => {
    ...
    client.user.setActivity(`Music cloud ☁️`,{type: 'listening'}); ///التعديل علي البلاينج
});
...
const command = args.shift().toLowerCase();
    ...
    if(cmd === 'play') {
        ...
        if(!voiceChannel) return msg.channel.send(`<:fales:763175742561189909> You must be listening in a voice channel to use that!`);
        const permissions = voiceChannel.permissionsFor(msg.client.user);
        if(!permissions.has('CONNECT')) {
            return msg.channel.send(`<:fales:763175742561189909> I can't join Your voiceChannel because i don't have ` + '`' + '`CONNECT`' + '`' + ` permission!`);
        }

        if(!permissions.has('SPEAK')) {
            return msg.channel.send(`<:fales:763175742561189909> I can't SPEAK in your voiceChannel because i don't have ` + '`' + '`SPEAK`' + '`' + ` permission!`);
        }
        ...
        if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			try {
				...
				try {
					...
					console.error(err);
					return msg.channel.send('<:fales:763175742561189909> I can\'t find any thing');
				}
			}
  ...
  let ms = video.durationSeconds * 1000;
	...
	if (!serverQueue) {
      ...
      setTimeout(() => {//:watch: Loading... [let]
        m.edit(`<a:voice:762345379122511902> Added **${song.title}**` + '(` ' + song.duration + ')`' + ` to the queue at position ` + `${serverQueue.songs.length}`);
      }, 500)
    }) 
	}
	return undefined;
...
function play(guild, song) {
  ...
  if(serverQueue.repeating) {
	...
	serverQueue.textChannel.send('<a:voice:762345379122511902> Added **' + song.title + '** (`' + song.duration + '`) to begin playing.');
}
	...
	dispatcher.setVolumeLogarithmic(serverQueue.volume / 100);
        ...
        return msg.channel.send('<a:voice:762345379122511902> The player has stopped and the queue has been cleared.');
      ...
      let queue = active.get(msg.guild.id);
        ...
        if(req == 1) {
            msg.channel.send('**<a:voice:762345379122511902> Skipped **' + args);
      ...
      let queue = active.get(msg.guild.id);
        ...
        if(!queue.playing) return msg.channel.send('<:fales:763175742561189909> There must be music playing to use that!')
        ...
        msg.channel.send('<a:voice:762345379122511902> Paused ' + args + '. **Type** `' + prefix + 'resume` to unpause!')
      ...
      let queue = active.get(msg.guild.id);
        ...
        if(!queue) return msg.channel.send('<a:voice:762345379122511902> No music paused to resume.')

        if(queue.playing) return msg.channel.send('<a:voice:762345379122511902> No music paused to resume.')
        ...
        msg.channel.send('<a:voice:762345379122511902> Resumed.')

    } else if(cmd === 'volume') {

      let queue = active.get(msg.guild.id);

      if(!queue || !queue.songs) return msg.channel.send('<a:voice:762345379122511902> There is no music playing to set volume.');

      let vCh = msg.member.voiceChannel;

      if(!vCh || vCh !== msg.guild.me.voiceChannel) return msg.channel.send('<a:voice:762345379122511902> You are not in my voice channel');

      let disp = queue.connection.dispatcher;

      if(isNaN(args[0])) return msg.channel.send('<a:voice:762345379122511902> Numbers only!');
...
//:speaker: Volume changed from 20 to 20 ! The volume has been changed from ${queue.volume} to ${args[0]}
      ...
      if(!queue) return msg.channel.send('<:fales:763175742561189909> There must be music playing to use that!');
      ...
      if(queue.repeating) {
        ...
        return msg.channel.send('<a:loading:762345385967878160> **Repeating Mode** (`False`)');
      } else {
        queue.repeating = true;
        return msg.channel.send('<a:loading:762345385967878160> **Repeating Mode** (`True`)');
      }
